...
def get_arguments() -> tuple[str, list[list[int]] | None]:
    if len(sys.argv) < 2:
...
def extract_shapes(filename:str, pal0:list[list[int]] | None = None):
...
def read_palette(handle:io.BufferedReader, size:int=256) -> list[list[int]]:
    entries = []
...
def shp_to_png(dir:str, palette:list[list[int]] | None, handle:io.BufferedReader, entry:int, total):
